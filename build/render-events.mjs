...
for(const [i,e] of EVENTS.entries()){
 ...
 const faqs=[
  ['Dove e quando si svolge l’incontro?',`L’incontro si svolge il ${e.label} alle 18:30, ora locale di Milano (Europe/Rome), presso l’Ufficio Bebit, Via Rutilia 10, 20141 Milano.`],
...
<header class="header"><a href="/" aria-label="NIUEXA, consulenza AI">${logo}</a><span class="header-edition">INCONTRI / AUTUNNO 2026</span><a class="header-link" href="${hub}">Tutti gli incontri <span aria-hidden="true">↗</span></a></header>
<main>
...
<aside class="event-facts" aria-label="Informazioni e partecipazione"><p class="eyebrow">INCONTRO ${String(i+1).padStart(2,'0')} / AUTUNNO 2026</p><time datetime="${e.date}T18:30:00${e.offset}">${e.label}</time><p class="event-time">Ore 18:30 <span>Europe/Rome</span></p><p><strong>Ufficio Bebit</strong><br>Via Rutilia 10, 20141 Milano</p>${cta(e)}<p class="cta-note">La richiesta non equivale alla conferma del posto.</p></aside></div>
</div></section>
...
html=html.replace(/<div class="dates" aria-label="Date degli incontri">[\s\S]*?<\/div>\s*<\/section>/,`<div class="dates" aria-label="Date degli incontri">\n${EVENTS.map(e=>`<article class="topic-card"><a href="${path(e)}" class="calendar-date"><span class="month">${e.month}</span><span class="day">${e.day}</span><span class="date-description">${e.title}<small>${e.label} · 18:30 · Bebit · Milano</small></span><span class="date-arrow" aria-hidden="true">↗</span></a><p>${e.excerpt}</p><div class="topic-actions"><a href="${path(e)}" aria-label="Scopra l’incontro: ${e.title}">Scopra l’incontro</a><a href="${signup(e)}" aria-label="Richieda la partecipazione: ${e.title}">Richieda la partecipazione ↗</a></div></article>`).join('\n')}\n </div>\n</section>`);
html=html.replace(/<section class="event-info programme"[\s\S]*?<\/section>/,`<section class="event-info programme" aria-labelledby="programme-title"><div><p class="eyebrow">UN PERCORSO, TRE PROSPETTIVE</p><h2 id="programme-title">Dalla visibilità<br>al lavoro quotidiano.</h2><p>Tre incontri per collegare le domande del mercato alle scelte operative dell’impresa. Il filo conduttore è uno: partire dai processi e dalle persone, prima degli strumenti.</p></div><ol class="programme-list"><li><h3>Farsi riconoscere</h3><p>A ottobre, come rendere comprensibile l’offerta aziendale nella ricerca e nelle risposte AI.</p></li><li><h3>Scegliere dove intervenire</h3><p>A novembre, come individuare i processi da migliorare e definire criteri per misurare il valore.</p></li><li><h3>Collegare senza perdere il controllo</h3><p>A dicembre, come integrare agenti, dati e strumenti con responsabilità e supervisione umana.</p></li></ol></section>`);
for(const e of EVENTS) html=html.replace(new RegExp(`<option value="${e.date}">[^<]*<\\/option>`),`<option value="${e.date}">${e.label} · ${e.title}</option>`);
...
llms=llms.replace(/## Eventi AI per aziende\n[\s\S]*?\n## Core Services/,`## Eventi AI per aziende\n- [Incontri NIUEXA, autunno 2026](https://niuexa.ai/eventi-ai-aziende/): modulo unico per richiedere la partecipazione, non conferma del posto. Tutte le date alle 18:30 (Europe/Rome), Ufficio Bebit, Via Rutilia 10, 20141 Milano.\n${EVENTS.map(e=>`- [${e.title}](https://niuexa.ai${path(e)}): ${e.label}. ${e.excerpt}`).join('\n')}\n\n## Core Services`);
write('llms.txt',llms);write('llm.txt',llms);
console.log(`Rendered ${EVENTS.length} event pages, hub topics and sitemap.`);
