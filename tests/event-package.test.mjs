...
test('Pages builds the actual allowlisted release artifact without asserting legal approval',()=>{
 ...
 const included=['index.html','eventi-ai-aziende/index.html','event-registration.mjs','assets/event-fonts/hanken-regular.ttf','assets/event-fonts/Hanken-OFL.txt','sitemap.xml','CNAME','en/llms.txt','en/sitemap.xml','en/robots.txt','img/ufficibebitmilano.2jpg','ai-metric-contract.html','img/landing/niuexa-session-milano-800.webp','landing-niuexa-v1.html'];
...
test('nested event package allows only known pages, resolves links and rejects private JSON/HTML',()=>{
 const fixtures=['eventi-ai-aziende/qa-private.json','eventi-ai-aziende/qa-private.html','eventi-ai-aziende/seo-in-pensione-aeo-geo/qa-private.json','eventi-ai-aziende/farsi-trovare-era-ai/qa-private.html'];
 try {
  ...
  // Retired slugs must ship as redirects, or old links would 404 on Pages.
  for(const [from,to] of MOVED){
   const path=`eventi-ai-aziende/${from}/index.html`;
   assert.equal(readFileSync(resolve(root,'_site',path),'utf8'),readFileSync(resolve(root,path),'utf8'),path);
   assert.ok(existsSync(resolve(root,'_site',`eventi-ai-aziende/${to}/index.html`)),`${path} -> ${to}`);
  }
